    ...
    def __getitem__(self, idx: int):
        if self.with_attributes:
            if self.concat_static:
                x = torch.cat(
                    [self.x[idx], self.attributes.repeat((self.seq_length, 1))],
                    dim=-1,
                )
                return x, self.y[idx]
            else:
...
class CamelsH5(Dataset):
    ...
    Should be used only in combination with the files processed from `create_h5_files`
    in the `papercode.utils` module.
    """
    ...
    def _preload_data(self):
        with h5py.File(self.h5_file, "r") as f:
            x = f["input_data"][:]
            y = f["target_data"][:]
            str_arr = [s.decode("ascii") for s in f["sample_2_basin"][:]]
            q_stds = f["q_stds"][:]
        return x, y, str_arr, q_stds

    def _get_basins(self):
        if self.cache:
            return list(set(self.sample_2_basin))
        else:
            with h5py.File(self.h5_file, "r") as f:
                str_arr = [s.decode("ascii") for s in f["sample_2_basin"][:]]
